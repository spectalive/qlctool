...
def _matrices(root):
    return [f for f in iter_local(root, "Function") if f.attrib.get("Type") == "RGBMatrix"]
...
def test_builder_reproduces_every_real_matrix(show):
    ...
    for original in originals:
        ...
        rebuilt = build_rgbmatrix(
            ...
            properties={
                p.attrib["Name"]: p.attrib["Value"] for p in findall_local(original, "Property")
            },
...
def test_fixture_groups_read_only_definitions():
    ...
    assert [(g.group_id, g.name) for g in groups] == [(0, "BarrasLed"), (1, "Cabezas"), (2, "PAR")]
    bars = groups[0]
...
def test_generate_matrix_effects_into_real_show(tmp_path):
    ...
    result = generate_matrix_effects(ws, group_id=0, algorithms=algorithms, palette=palette)
    ...
    generated = {
        m.attrib["Name"]: m for m in matrices if int(m.attrib["ID"]) in set(result.matrix_ids)
    }
...
def test_generation_matches_the_target_show_colour_shape(tmp_path):
    ...
    generated = next(
        f
        for f in _matrices(Workspace.load(out).root)
...
def test_curated_scripts_carry_their_own_properties(tmp_path):
    ...
    matrix = next(m for m in _matrices(reloaded) if int(m.attrib["ID"]) == result.matrix_ids[0])
    assert matrix.attrib["Name"] == "BarrasLed - Sine Wave Rojo/Azul"
    properties = {p.attrib["Name"]: p.attrib["Value"] for p in findall_local(matrix, "Property")}
...
def test_curated_scripts_are_stepped_for_a_full_pass_each():
    ...
    chaser = next(
        f for f in iter_local(ws.root, "Function") if f.attrib.get("ID") == str(result.chaser_id)
    )
    steps = {int(s.text): int(s.attrib["Hold"]) for s in findall_local(chaser, "Step")}
