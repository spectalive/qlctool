...
def _scenes(workspace: Workspace) -> list[Values]:
    """Every scene's values, fixture id -> {channel offset: value}."""
    scenes = []
...
def built(request, tmp_path_factory) -> tuple[list[FixtureCapabilities], list[Values]]:
    source = VIBRA
    ...
    return capabilities_of(workspace.root, library), _scenes(workspace)
...
def test_2026_09_25_the_levels_hold_the_lit_smoke_led_at_full_and_the_pump_at_zero(built):
    ...
    for machine in (c for c in caps if c.is_lit_smoke):
        fixture = machine.fixture.fixture_id
        levels = [
            s
            for s in scenes
            if _writes(machine, s, (roles.DIMMER,)) and not _writes(machine, s, RGB)
        ]
        held = [
            s
            for s in levels
            if all(s[fixture][o] == 255 for o in machine.offsets_for_role(roles.DIMMER))
            and all(s[fixture].get(o) == 0 for o in fog_offsets(machine))
        ]
        assert held, machine.fixture.name
