...
| `humo pegado` | A Flash button that raises a smoke pump on a channel some room state never writes. The pump is LTP: release restores nothing, so the machine fogs until somebody reloads the show | "Le doy y nunca se para, se supone que solo debe tirar cuando le de": `Humo Vertical YA` opened the Fog channel and no state wrote it back (2026-08-29) |
| `figura fuera del publico` | An EFX whose beam figure - centre plus half-size, per axis - leaves the pan/tilt window where the audience actually is | The aim was guessed twice the same night, landing on the floor and then on the wall, until the owner read the corners off the desk: pan 62-103, tilt 207-234 on head #1 (2026-08-29) |
