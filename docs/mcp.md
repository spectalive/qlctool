...
```bash
pip install "qlctool[mcp] @ git+https://github.com/spectalive/qlctool.git"
# or, in a clone
...
| `validate` | `workspace` | `ok` and QLC+'s complaints, from a headless QLC+ of the server's own |
...
| Tool | Arguments | Returns |
| --- | --- | --- |
| `live_status` | `workspace?` | `reachable`, `project_loaded`, the function and widget counts, `writes_allowed`; with `workspace`, whether the running show is that file |
...
| `live_press` | `widget_id`, `value=255` | the widget's state `before` and after sending `<widget_id>\|<value>` |
| `live_function` | `function_id`, `state` (`on` or `off`) | whether the function is running afterwards |
...
`live_press` sends what the web console sends. A flash button takes 255 as a
...
- **Validation uses its own QLC+.** `validate` starts and stops a QLC+ of its
  own; a QLC+ somebody already has open stays open.
