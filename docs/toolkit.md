...
| `newshow` | A whole self-running show built on an existing patch, plus the four-page live console (show / JUGAR / control / library) on one screen. A patch with no fixture group is refused before anything is generated, with a message saying so; one whose definitions are not found is told that instead. A rig with nothing that pans and tilts gets no movement, and one with no fader dimmer no dimmer chase |
