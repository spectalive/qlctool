...
| `newshow` | A whole self-running show built on an existing patch, plus the four-page live console (show / JUGAR / control / library) on one screen. A patch with no pan/tilt fixture, no fixture with a fader dimmer (outside a pixel group and not running its own programmes), or no fixture group is refused before anything is generated, with a message naming what it lacks; one whose definitions are not found is told that instead |
